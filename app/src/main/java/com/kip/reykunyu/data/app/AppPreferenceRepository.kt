...
class AppPreferenceRepository(
    ...
    val dialect: Flow<Dialect> = dataStore.data
        .catch {
            if(it is IOException) {
                Log.e(TAG, "Error reading preferences.", it)
                emit(emptyPreferences())
        ...
        .map { preferences ->
            //Defaults to english
            preferences[DIALECT]?.let { Dialect.fromCode(it) } ?: Dialect.Combined
        }
    ...
    suspend fun updateDialectPreference(
        dialect: Dialect
    ) {
        val targetKey = DIALECT

        val targetValue = dialect.toString()
